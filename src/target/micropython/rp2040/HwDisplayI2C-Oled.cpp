...
void writeCmd(uint8_t *buf, int len)
{
	...
	i2cproto->transfer_single(i2c, DISPLAY_ADDRESS, len + 1, newBuf, i2cFlags);
}
...
void hwDisplaySetup()
{
	...
	mp_obj_t freq = mp_obj_new_int(2000000);
	...
	writeCmd(INIT_COMMANDS, INIT_COMMANDS_SIZE);
	...
	for (int page = 0; page < 8; page++)
	{
		...
		writeCmd(setPageCmd, 3);
		writeData(tempBuff, DISPLAY_COLUMNS);
	}
}
...
void hwDisplaySend()
{
	for (int page = 0; page < 8; page++)
	{
		...
		writeCmd(setPageCmd, 3);
		writeData(displayBuffer + (page * 128), 128);
	}
}
